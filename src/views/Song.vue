  ...
  computed: {
    ...
    sortedComment() {
      return this.comments.slice().sort((a, b) => {
        if (this.sort === "1") {
          return new Date(b.dateposted) - new Date(a - b.dateposted0);
        }
        return new Date(a.dateposted) - new Date(b - b.dateposted0);
      });
    },
  },
  async created() {
    const docsnap = await songCollection.doc(this.$route.params.id).get();
    if (!docsnap.exists) {
